...
from svgd_marginal import SVGD
import sys
...
class StateSpaceModel:
    ...
    def lnprob_theta_i(self, theta_i, theta_t_minus_1, time_series,t,eta):
        #ln poisson observations
            ...
            for theta_t_minus_1_i in theta_t_minus_1:
                ...
                transition_sum += 1.0/(np.sqrt(2*np.pi*eta))*np.exp(-.5*(1.0/eta)*((theta_i - theta_t_minus_1_i )**2))
                
            return (lnprob_theta_i+np.log(transition_sum))
    
    def dlnprob(self, theta_i,theta_t_minus_1,time_series, t,eta ):
        return (grad(self.lnprob_theta_i)(theta_i, theta_t_minus_1, time_series,t,eta))
    
    def grad_overall(self, theta,theta_t_minus_1,time_series, t,eta):
        return_matrix = []

        for theta_i in theta:
            return_matrix.append(self.dlnprob(theta_i,theta_t_minus_1 ,time_series,t,eta))
    
        return np.array(return_matrix)
    

# We now have a method to get the likelihood of a specific eta setting  

def get_bootstrap_log_likelihood(eta):
    ...
    theta = SVGD().update(x0,0,x0,time_series, model.grad_overall, eta, n_iter=n_iter, stepsize=0.01)
    ...
    for t in range(1,len(time_series)):
      theta = SVGD().update(theta,t,theta, time_series, model.grad_overall,eta, n_iter=n_iter, stepsize=0.01)
      total_thetas.append(theta)
      filtered_means.append(np.mean(theta,axis=0)[0])
      filtered_covs.append(np.var(theta,axis=0)[0])
    
    # i guess this is just the ith sample??? 

    total_filter_log_likelihood = 0
    for i in range(len(time_series)):
        tmp_likelihood = 0
        for j in range(len(total_thetas[i])):
            tmp_likelihood += norm.pdf(time_series[i],total_thetas[i][j],1)
        
        print (tmp_likelihood)
        total_filter_log_likelihood += np.log(1.0/len(total_thetas[i])*tmp_likelihood)   
    return total_filter_log_likelihood



proposal_width = 10
# NOW WE ARE READY FOR METROPOLIS HASTINGS!!!!!!
posterior_samples = []
for i in range(100):
    mu_current = 1.
    ...
    # use a bootstrap estimate of the log likelihood
    # PROBLEM: NO IDEA IF THIS IS UNBIASED OR NOT  
    log_likelihood_current = get_bootstrap_log_likelihood(mu_current)
    log_likelihood_proposal =get_bootstrap_log_likelihood(proposal)

    # WHAT SHOULD THE BANDWITH PRIOR BE    
    log_prior_current = np.log(norm(mu_prior_mu, mu_prior_sd).pdf(mu_current))
    log_prior_proposal = np.log(norm(mu_prior_mu, mu_prior_sd).pdf(proposal))
    ...
    log_accept = min(0, log_p_accept)
    accept = np.random.rand() < log_accept
    if accept:
        # Update position
